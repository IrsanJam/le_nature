...
const CardProduct: FC<CardProps> = ({ title, description, price, originalPrice, image, directId, bestSeler }) => {
  return (
    <div className="flex flex-col  mx-auto my-5 lg:pb-10 md:h-[80vh] h-[60vh] w-[90%] bg-white rounded-lg  shadow-lg font-Poppins">
      ...
      <div onClick={directId} className="tracking-wide mt-6 hover:scale-105 hover:font-bold hover:px-4 cursor-pointer text-sm md:text-lg font-bold leading-7 px-2 text-zinc-600">
        {title}
      </div>
      <div className="mt-3.5 md:text-sm text-xs  leading-5 tracking-wide font-light text-zinc-500 px-2">{description}</div>
      <div className="flex gap-5 justify-between mt-5 w-full whitespace-nowrap">
        <div className="flex md:gap-3 gap-[-20px] my-auto pb-2">
        ...
        <img
          ...
          className="shrink-0 w-11 border-solid aspect-square md:pr-2 hover:scale-105 z-[999]"
        />
      </div>
    </div>
  );
